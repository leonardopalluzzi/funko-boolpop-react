...
export default function ProductPage() {
  ...
  const { handleCart, cart, subtractCartQuantity, addCartQuantity } =
    useCartContext();
  ...
  useEffect(() => {
    if (funko.state === "success") {
      const foundItem = cart.userCart.find(
        (item) => item.slug === funko.result.slug
      );
  ...
  useEffect(() => {
    fetch(
      `http://localhost:3000/api/v1/recommended?slug=${pageSlug}&limit=${limit}&page=${pageDate}`
    )
      .then((res) => res.json())
      .then((data) => {
        setSuggested({
            ...
            <div className="container py-5 h-100">
              <div className="row row-cols-1 row-cols-lg-2 ">
                ...
                <div className="col">
                  ...
                  <button
                    onClick={() =>
                      handleCart(funko.result, funko.result.quantity)
                    }
                    ...
                    {cartItem ? (
                      <>
                        <button
                          ...
                          onClick={() => subtractCartQuantity(cartItem)}
                        >
                          -
                        </button>
                        ...
                        <button
                          className="btn btn-transparent"
                          onClick={() =>
                            addCartQuantity(cartItem, funko.result.quantity)
                          }
                        >
                          +
                        </button>
                  ...
                  <span className="d-block pt-4">
                    {" "}
                    <i class="bi bi-box-fill"></i> Disponibilità:{" "}
                    {cartItem != null
                      ? cartItem.quantity
                      : funko.result.quantity}
                  </span>
              ...
              {suggested.state === "success" ? (
                <>
                  <div className="carousel_productpage mb-5">
                    ...
                    <CarouselUi
                      dataLength={suggested.results.totalPages}
                      page={pageDate}
                      setPage={setPageDate}
                      content={
                        <>
                          <List
                            products={suggested.results}
                            queryName={"date"}
                            page={pageDate}
                            query={date}
                          />
