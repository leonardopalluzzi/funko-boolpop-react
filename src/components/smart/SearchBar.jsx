      ...
      return (
        <>
          ...
          <p
            className={
              message !== ""
                ? `alert alert-danger position-absolute top-0 left-50`
                : "d-none"
            }
          >
            {message}
          ...
          <div className={display == false ? "d-block" : "d-none"}>
            {filteredFunko.data.results &&
            filteredFunko.data.results.length > 0 ? (
              <>
                <SearchResultsUi
                  emptyResearch={emptyResearch}
                  results={filteredFunko.data.results}
                />
              </>
            ) : (
              <></>
